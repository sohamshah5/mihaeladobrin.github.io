    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin], {
      ...
      displayPopupContent: function (feature) {
        if (feature) {
          if(this.inPanel){
            if(this.appConfig.theme.name === 'JewelryBoxTheme'){
              var oCenter = this.map.extent.getCenter();
              PanelManager.getInstance().maximizePanel(this.inPanel);
              setTimeout(lang.hitch(this, function(){
                this.reCenterMap(feature, oCenter);
              }), 400);
            }else if(this.appConfig.theme.name === 'TabTheme') {
              var sbc = WidgetManager.getInstance().getWidgetsByName("SidebarController")[0];
      ...
      reCenterMap: function(feature, oCenter) {
        if(feature.geometry){
          var sCenter = this.map.toScreen(oCenter)
          var w = this.inPanel.maxWidth / 2;
          var asX = sCenter.x + w;
          var aX = this.map.toMap(new ScreenPoint(asX, sCenter.y));
          if(feature.geometry.type === 'point'){
            if(!this.map.extent.contains(feature.geometry)){
              this.map.centerAt(aX);
            }
          }else if(feature.geometry.getExtent){
            var cent = feature.geometry.getExtent();
            if(!this.map.extent.contains(cent)){
              this.map.centerAt(aX);
            }
          }
        }
